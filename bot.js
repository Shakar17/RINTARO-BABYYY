...
function configuredBots() {
  return Array.from({ length: maxBots }, (_, index) => ({
    number: index + 1,
...
async function runWebControl(action, guildIdToControl, channelIdToControl, filename, loop = false, volume = 100) {
  ...
  if (!activeBots.length) {
    ...
    throw new Error(`No bots are online. ${details || `Add DISCORD_TOKEN_1 through DISCORD_TOKEN_${maxBots} in Render.`}`);
  }
...
addLog('info', `Configured ${tokenCount}/${maxBots} Discord bot token(s).`);
if (!tokenCount) addLog('error', `No Discord bot tokens configured. Add DISCORD_TOKEN_1 through DISCORD_TOKEN_${maxBots} in Render.`);
